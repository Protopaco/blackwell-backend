...
describe('GET /api/v1/payPeriod', () => {
  it('returns 200 with a list of pay periods', async () => {
    const res = await request(app).get(`/api/v1/payPeriod?clientId=${TEST_CLIENT_ID}`);
  ...
  it('returns 200 with empty array for unknown client', async () => {
    const res = await request(app).get('/api/v1/payPeriod?clientId=unknown-id');
    expect(res.status).toBe(200);
