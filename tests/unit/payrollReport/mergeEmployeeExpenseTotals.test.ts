...
describe('mergeEmployeeExpenseTotals', () => {
  it('overlays totalExpense onto a matching record, preserving employeeName and activeThisPayPeriod', () => {
  ...
  it('updates only the matching records when some employees are updated and others are not', () => {
    const merged = mergeEmployeeExpenseTotals(
      [
        { employeeId: 'e1', employeeName: 'Jane Smith', activeThisPayPeriod: true, totalExpense: 100 },
        { employeeId: 'e2', employeeName: 'John Doe', activeThisPayPeriod: false, totalExpense: 50 },
      ],
      [{ employeeId: 'e1', totalExpense: 200 }],
    );
