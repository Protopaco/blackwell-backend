...
describe('GET /api/v1/payrollReport/:clientId/:payPeriodId/additionalExpenses', () => {
  it('200 - Gets additional expenses for a generated payroll report', async () => {
  ...
  it('404 - Client not found', async () => {
    ...
    const res = await request(app).get(
      `/api/v1/payrollReport/${missingClientId}/${payPeriodId}/additionalExpenses`,
    );
  ...
  it('404 - Pay period not found', async () => {
    ...
    const res = await request(app).get(
      `/api/v1/payrollReport/${client.clientId}/${missingPayPeriodId}/additionalExpenses`,
    );
