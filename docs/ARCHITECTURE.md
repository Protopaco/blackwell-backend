...
- **Pay-period config snapshots are frozen on purpose, by design, not a gap.** `createPayPeriodConfigSnapshot.ts` seeds a point-in-time copy of the client's full `PayrollConfig` (Employees/Activities/EmployeeActivityRates/FundingSources/Holidays/Settings) into the pay period's own `payrollReportFileId` workbook at creation time. General client-wide config CRUD (`updateEmployee`, `createActivity`, `updateActivity`, `deleteActivity`, `createEmployee`, funding-source/holiday CRUD) only ever touches the live `Client.payrollConfigFileId` and deliberately never reaches into an already-created pay period's embedded snapshot — each pay period is meant to stay a stable record of config-as-of-creation, required for payroll correctness (a pay period shouldn't retroactively change what it paid against). Only the explicit `addXToPayPeriod` / `removeXFromPayPeriod` / `updateActivityOnPayPeriod` / `syncHolidaysOnPayPeriod` family of services re-syncs an *existing* pay period's embedded config, and each of those correctly invalidates `payPeriodConfigSnapshotCache` (24h TTL, keyed by `payrollReportFileId`) when it writes. Don't add snapshot-cache invalidation to the general CRUD services above — confirmed 2026-08-13 that it would be inert, since none of them write into a pay period's embedded tabs at all.
...
- **Two timesheet layouts, chosen by `Settings.timeInputMethod`** (added 2026-08-13): `TimesheetManifest` has both a `weeks: WeekManifest[]` field (TotalHours mode — stacked per-week sections) and an optional `clockInOutWeeks?: ClockInOutWeekManifest[]` field (ClockInOut mode — weeks laid out side by side as column groups, each day holding fixed entry-slot rows plus an optional per-day Flat Rate section). A given timesheet is consistently one shape or the other, never mixed. `generateTimesheets.ts` branches on `timeInputMethod` to call `buildWeek.ts` or `buildClockInOutTimesheet.ts`; `readTimesheetEntries.ts` branches on which manifest field is populated to call `readTotalHoursEntries.ts` or `readClockInOutEntries.ts` (which further delegates to `readClockInOutSlotRows.ts` for Hourly slots and `readClockInOutFlatRateRows.ts` for Flat Rate rows). Both read paths produce the same mode-agnostic `TimesheetEntry[]` shape, so `generatePayrollReport.ts`/`buildHoursRows.ts`/`buildSummaryRows.ts` need no branching at all. See `docs/BUSINESS_RULES.md`'s "ClockInOut Timesheets" section and `docs/TIMESHEET_STYLE_GUIDE.md`'s "ClockInOut Timesheets" section for the full row/validation/styling detail.
