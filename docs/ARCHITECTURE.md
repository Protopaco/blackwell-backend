...
- `Clients` tab: `ClientId`, `ClientName`, `ClientCode`, `Status` (`Active`/`Inactive`), `EmployeePayrollFolderId`, `PayrollConfigFolderId`, `PayrollReportFolderId`, `TimesheetsFolderId` (nullable), `PayrollConfigFileId`, `PayPeriodRegistryFileId`.
...
- `TimesheetsFolderId` being nullable is a separate, still-open concern from the rest of Client CRUD — see "Deferred / intentionally out of scope" below.
...
- **`TimesheetsFolderId` resolution** — nullable by design (some clients' timesheet folders live outside the standard Employee Payroll tree, with more variable structure than the rest of Client CRUD assumed). `createClient.ts` accepts it as an optional plain ID with no verification. Still open: how a client with a `null` value gets one resolved later (likely prompting the user at first timesheet generation, not at client creation), and the matching permission-check work that implies (verifying access to a folder the app doesn't control) — `createEmployee.ts` and `generateTimesheets.ts` currently just throw `UnprocessableError` if it's null when a new timesheet file would be needed, as a stopgap, not the real design.
