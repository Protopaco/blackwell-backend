...
**Pattern**: the `db/<domain>/read*.ts` function checks its cache first, populates it on a miss. Whichever `db/<domain>/write*.ts` (or generation service) invalidates that data calls `<cache>.delete(key)` right after a successful write — always a single, trivial call, never conditional logic. `clientsCache` has no invalidation because nothing currently writes to the `Clients` tab (client create/edit is unbuilt "phase two" work — see below).
...
- `Employees`: `EmployeeId`, `FirstName`, `LastName`, `Position`, `HourlyPayRate1`, `HourlyPayRate2`, `HolidayPayRate`, `Email`, `Status` (`Active`/`Inactive`), `TimesheetFileId`, plus a still-live but unused `TimesheetFileLink` column (app stopped reading/writing it 2026-07-12, left in place rather than migrated). Hourly/holiday pay rates ARE used in calculations (allocation proportion weighting via `buildAllocationRows.ts`) — not display-only. `Employee.flatPayRate1`/`flatPayRate2` were removed 2026-07-13 (confirmed dead — no live sheet column ever existed for them); the dollar amount for flat-rate work belongs on `Activity.flatRateAmount` instead, but `buildAllocationRows.ts` isn't wired up to use it yet (resolves to $0 for `FlatPayRate1`/`FlatPayRate2` activities — see `docs/TODO.md`).
...
- **Client create/edit** — no write path exists for the `Clients` tab at all (`src/db/client/` is read-only). This is "phase two" — configuration UIs for clients/employees/activities/etc. currently live in the pre-existing Apps Script tool, not this service. `clientsCache` correspondingly has no invalidation logic (nothing to invalidate).
