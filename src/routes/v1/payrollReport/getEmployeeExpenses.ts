...
router.get('/:clientId/:payPeriodId/employee-expenses', async (req: Request, res: Response) => {
  ...
  if (!expenses) {
    logger.info(`GET /payrollReport/${clientId}/${payPeriodId}/employee-expenses — response 404`);
    return res.status(404).json({ error: 'not_found', message: 'Client, pay period, or payroll report not found' });
  }
